...
// ---------------------------------------------------------------------------
// Custom sqrt and rsqrt using v_sqrt_f32 and v_rsq_f32
// ---------------------------------------------------------------------------
// clang-format off
inline float __device__ custom_sqrt_small_neg_wrong(float x) {
  float result;
  __asm__ __volatile__("v_sqrt_f32_e32 %0, %1\n\t"
                       : "=v"(result) // %0
                       : "v"(x)       // %1
                      );
  return result;
...
inline float __device__ custom_rsqrt_small_neg_wrong(float x) {
  float result;
  __asm__ __volatile__("v_rsq_f32_e32 %0, %1\n\t"
                       : "=v"(result) // %0
                       : "v"(x)       // %1
                      );
  return result;
...
inline float __device__ custom_sqrt(float x) {
  ...
  __asm__ __volatile__(
      "v_cmp_gt_f32_e64 vcc, 0.0, %2              ;; vcc = (x < 0.0)\n\t"
      "v_and_b32_e32 %1, 0x7fffffff, %2           ;; %1 = |x|\n\t"
      "v_sqrt_f32_e32 %0, %1                      ;; %0 = sqrt(|x|)\n\t"
      "v_mov_b32_e32 %1, 0xffc00000               ;; -NaN\n\t"
      "v_cndmask_b32_e64 %1, 1.0, %1, vcc         ;; %1 = x<0 ? NaN : 1.0\n\t"
      "v_mul_f32_e32 %0, %1, %0                   ;; result = %1 * sqrt(|x|)\n\t"
      : "=v"(result),  // %0 no early clobber, written only after all inputs are consumed.
        "=v"(oneOrNan) // %1 no early clobber, written only after all inputs are consumed.
      : "v"(x)         // %2
      : "vcc"
  );
...
inline float __device__ custom_sqrt_rocm_like_vcc(float x) {
  float result;
  float scaled;
  float est;
  float lo, hi;
  float rlo, rhi;
  uint64_t cmp_lo, cmp_hi;
  uint32_t class_mask = 0x260;

  __asm__ __volatile__(
      "                                       ;; Scale up small values: if x < 0x0f800000, multiply by 2^32\n\t"
      "v_mov_b32_e32 %1, 0x0f800000           ;; tmp = threshold\n\t"
      "v_mul_f32_e32 %2, 0x4f800000, %9       ;; scaled = x * 2^32\n\t"
      "v_cmp_lt_f32_e32 vcc, %9, %1           ;; vcc = (x < threshold)\n\t"
      "s_nop 1\n\t"
      "v_cndmask_b32_e32 %1, %9, %2, vcc      ;; scaled = (x < thr) ? x*2^32 : x\n\t"

      "                                       ;; Initial sqrt estimate\n\t"
      "v_sqrt_f32_e32 %2, %1                  ;; est = sqrt(scaled)\n\t"
      "s_nop 1                                ;; TRANS hazard\n\t"

      "                                       ;; Newton-Raphson fixup: round to nearest integer sqrt\n\t"
      "v_add_u32_e32 %3, -1, %2               ;; lo = est - 1\n\t"
      "v_add_u32_e32 %4, 1, %2                ;; hi = est + 1\n\t"
      "v_fma_f32 %5, -%3, %2, %1              ;; rlo = scaled - lo*est\n\t"
      "v_fma_f32 %6, -%4, %2, %1              ;; rhi = scaled - hi*est\n\t"
      "v_cmp_ge_f32_e64 %7, 0, %5             ;; cmp_lo = (0 >= rlo)\n\t"
      "s_nop 1\n\t"
      "v_cndmask_b32_e64 %2, %2, %3, %7       ;; est = (0>=rlo) ? lo : est\n\t"
      "v_cmp_lt_f32_e64 %8, 0, %6             ;; cmp_hi = (0 < rhi)\n\t"
      "s_nop 1\n\t"
      "v_cndmask_b32_e64 %2, %2, %4, %8       ;; est = (0<rhi) ? hi : est\n\t"

      "                                       ;; Undo scaling: vcc still holds (x < threshold) from above\n\t"
      "v_mul_f32_e32 %3, 0x37800000, %2       ;; lo = est * 2^-16\n\t"
      "v_cndmask_b32_e32 %2, %2, %3, vcc      ;; est = was_scaled ? est*2^-16 : est\n\t"

      "                                       ;; Pass through special values (0, inf, NaN): class mask 0x260\n\t"
      "v_cmp_class_f32_e64 vcc, %9, %10       ;; vcc = is_special(x)\n\t"
      "s_nop 1\n\t"
      "v_cndmask_b32_e32 %0, %2, %9, vcc      ;; result = special ? x : est\n\t"
...
inline float __device__ custom_sqrt_rocm_like_stemp(float x) {
  float result;
  float scaled;
  float est;
  float lo, hi;
  float rlo, rhi;
  uint64_t cmp_lo, cmp_hi, stemp;
  uint32_t class_mask = 0x260;

  __asm__ __volatile__(
      "                                       ;; Scale up small values: if x < 0x0f800000, multiply by 2^32\n\t"
      "v_mov_b32_e32 %1, 0x0f800000           ;; tmp = threshold\n\t"
      "v_mul_f32_e32 %2, 0x4f800000, %10      ;; scaled = x * 2^32\n\t"
      "v_cmp_lt_f32 %9, %10, %1               ;; stemp = (x < threshold)\n\t"
      "s_nop 1\n\t"
      "v_cndmask_b32 %1, %10, %2, %9          ;; scaled = (x < thr) ? x*2^32 : x\n\t"
      ...
      "                                       ;; Undo scaling: stemp still holds (x < threshold) from above\n\t"
      "v_mul_f32_e32 %3, 0x37800000, %2       ;; lo = est * 2^-16\n\t"
      "v_cndmask_b32 %2, %2, %3, %9           ;; est = was_scaled ? est*2^-16 : est\n\t"

      "                                       ;; Pass through special values (0, inf, NaN): class mask 0x260\n\t"
      "v_cmp_class_f32 %9, %10, %11           ;; stemp = is_special(x)\n\t"
      "s_nop 1\n\t"
      "v_cndmask_b32 %0, %2, %10, %9          ;; result = special ? x : est\n\t"
...
inline float __device__ custom_rsqrt_rocm_like_vcc(float x) {
  float result;
  float scaled;
  float tmp;

  __asm__ __volatile__(
      "v_mov_b32_e32 %2, 0x800000             ;; threshold = FLT_MIN\n\t"
      "v_mul_f32_e32 %1, 0x4b800000, %3       ;; scaled = x * 2^24\n\t"
      "v_cmp_lt_f32_e32 vcc, %3, %2           ;; vcc = (x < FLT_MIN)\n\t"
      "s_nop 1                                ;; vcc hazard\n\t"
      "v_cndmask_b32_e32 %1, %3, %1, vcc      ;; scaled = (x < FLT_MIN) ? x*2^24 : x\n\t"
      "v_rsq_f32_e32 %0, %1                   ;; result = rsqrt(scaled)\n\t"
      "v_mov_b32_e32 %2, 0x45800000           ;; tmp = 2^12 (TRANS hazard filler)\n\t"
      "v_mul_f32_e32 %2, %2, %0               ;; tmp = result * 2^12\n\t"
      "v_cndmask_b32_e32 %0, %0, %2, vcc      ;; result = was_scaled ? result*2^12 : result\n\t"
...
inline float __device__ custom_rsqrt_rocm_like_stemp(float x) {
  float result;
  float scaled;
  float tmp;
  uint64_t stemp;
  ...
  __asm__ __volatile__(
      "v_mov_b32_e32 %2, 0x800000             ;; threshold = FLT_MIN\n\t"
      "v_mul_f32_e32 %1, 0x4b800000, %4       ;; scaled = x * 2^24\n\t"
      "v_cmp_lt_f32 %3, %4, %2                ;; stemp = (x < FLT_MIN)\n\t"
      "s_nop 1                                ;; stemp hazard\n\t"
      "v_cndmask_b32 %1, %4, %1, %3           ;; scaled = (x < FLT_MIN) ? x*2^24 : x\n\t"
      "v_rsq_f32_e32 %0, %1                   ;; result = rsqrt(scaled)\n\t"
      "v_mov_b32_e32 %2, 0x45800000           ;; tmp = 2^12 (TRANS hazard filler)\n\t"
      "v_mul_f32_e32 %2, %2, %0               ;; tmp = result * 2^12\n\t"
      "v_cndmask_b32 %0, %0, %2, %3           ;; result = was_scaled ? result*2^12 : result\n\t"

      : "=v"(result),                         // %0 no early clobber, written only after all inputs are consumed.
        "=&v"(scaled),                        // %1
        "=&v"(tmp),                           // %2
        "=s"(stemp)                           // %3 no early clobber, scalar output cannot overlap vector inputs.
      : "v"(x)                                // %4
      : /* no clobbers */
  );
...
#define CUSTOM_SQRT custom_sqrt_small_neg_wrong
#define CUSTOM_RSQRT custom_rsqrt_small_neg_wrong
#else
// These two handle small negatives, but not scaling, and we can do both with very little extra cost.
//#define CUSTOM_SQRT custom_sqrt
//#define CUSTOM_RSQRT custom_rsqrt
//#define CUSTOM_SQRT custom_sqrt_rocm_like_vcc
//#define CUSTOM_RSQRT custom_rsqrt_rocm_like_vcc
#define CUSTOM_SQRT custom_sqrt_rocm_like_stemp
#define CUSTOM_RSQRT custom_rsqrt_rocm_like_stemp
#endif
